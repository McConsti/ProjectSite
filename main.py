...
@app.get("/quiz", response_class=HTMLResponse)  #Set the api path the quiz
def root(request: Request):
    return templates.TemplateResponse("index.html", {"request": request})

@app.get("/videos", response_class=HTMLResponse)  #You get how this works
def root(request: Request):
    return templates.TemplateResponse("vidplaybacktest.html", {"request": request})

if __name__ == '__main__':
    uvicorn.run(app, host='192.168.178.20', port=80) #My local ip so I can test easier